...
function positionSort() {
  ...
  if (ascendingPosition) {
    sortedPosition = [...people].sort((a, b) =>
  ...
  for (const person of sortedPosition) {
...
function nameSort() {
  ...
  for (const person of sortedName) {
    tableBody.append(person);
  }
}
...
function salarySort() {
  ...
  if (ascendingSalary) {
    sortetSalary = [...people].sort(
      (a, b) =>
        Number(a.children[3].textContent.replace(/[$,]/g, '')) -
        Number(b.children[3].textContent.replace(/[$,]/g, '')),
    );
  ...
  for (const person of sortetSalary) {
    tableBody.append(person);
  }
}
